...
public:
    ...
    error_t WaitAndAddOwner(uint32_t ms, SpuriousWakeup_f wakeup)    override;
...
private:
    error_t GoToSleep(uint32_t ms, SpuriousWakeup_f wakeup, bool workers);
    error_t NewThreadContext(WorkWaitingThreads * context, SpuriousWakeup_f wakeup, bool waiters);
