...
{
    _InterlockedIncrement((unsigned *)mm_struct_get_mm_users(mm));
}
...
{
    _InterlockedIncrement((unsigned *)task_get_usage(tsk));
}
...
void ProcessesTaskStructDec(task_k  tsk)
{
    ...
    should_exit = _InterlockedDecrement((unsigned *)task_get_usage(tsk)) == 0;
...
void OProcessThreadImpl::InvaildateImp()
{
...
void OProcessImpl::InvaildateImp()
{
    ProcessesTaskStructDec(_tsk);
